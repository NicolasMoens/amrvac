...
contains
  ...
  subroutine advance(iit)
    ...
    do iigrid=1,igridstail; igrid=igrids(iigrid);
       pw(igrid)%wold(ixGlo1:ixGhi1,1:nwflux+nwaux)=pw(igrid)%w(ixGlo1:ixGhi1,&
          1:nwflux+nwaux)
    end do
    !$OMP END PARALLEL DO
  ...
  subroutine advect(idimmin,idimmax)
    ...
    case ("ssprk43")
       ...
       ! === Second step ===
       do iigrid=1,igridstail; igrid=igrids(iigrid);
          pw(igrid)%w2(ixGlo1:ixGhi1,1:nwflux)=pw(igrid)%w1(ixGlo1:ixGhi1,&
             1:nwflux)
          if (nw>nwflux) pw(igrid)%w2(ixGlo1:ixGhi1,&
             nwflux+1:nw) = pw(igrid)%w(ixGlo1:ixGhi1,nwflux+1:nw)
       end do
       ...
       ! === Third step ===
       do iigrid=1,igridstail; igrid=igrids(iigrid);
          pw(igrid)%w3(ixGlo1:ixGhi1,1:nwflux)=2.0d0/3.0d0 * &
             pw(igrid)%w(ixGlo1:ixGhi1,1:nwflux) + 1.0d0/3.0d0 * &
             pw(igrid)%w2(ixGlo1:ixGhi1,1:nwflux)
          if (nw>nwflux) pw(igrid)%w3(ixGlo1:ixGhi1,&
             nwflux+1:nw) = pw(igrid)%w(ixGlo1:ixGhi1,nwflux+1:nw)
       end do
       ...
       !$OMP PARALLEL DO PRIVATE(igrid)
       do iigrid=1,igridstail_active; igrid=igrids_active(iigrid);
          pw(igrid)%w(ixGlo1:ixGhi1,1:nwflux)=pw(igrid)%w3(ixGlo1:ixGhi1,&
             1:nwflux)
       end do
       !$OMP END PARALLEL DO
    ...
    case ("ssprk54")
       ...
       ! === Second step ===
       do iigrid=1,igridstail; igrid=igrids(iigrid);
          pw(igrid)%w2(ixGlo1:ixGhi1,1:nwflux)=0.444370493651235d0 * &
             pw(igrid)%w(ixGlo1:ixGhi1,1:nwflux) + 0.555629506348765d0 * &
             pw(igrid)%w1(ixGlo1:ixGhi1,1:nwflux)
          if (nw>nwflux) pw(igrid)%w2(ixGlo1:ixGhi1,&
             nwflux+1:nw) = pw(igrid)%w(ixGlo1:ixGhi1,nwflux+1:nw)
       end do
       ...
       ! === Third step ===
       do iigrid=1,igridstail; igrid=igrids(iigrid);
          pw(igrid)%w3(ixGlo1:ixGhi1,1:nwflux)=0.620101851488403d0 * &
       ...
       ! === Fourth step ===
       do iigrid=1,igridstail; igrid=igrids(iigrid);
          pw(igrid)%w4(ixGlo1:ixGhi1,1:nwflux)=0.178079954393132d0 * &
             pw(igrid)%w(ixGlo1:ixGhi1,1:nwflux) + 0.821920045606868d0 * &
             pw(igrid)%w3(ixGlo1:ixGhi1,1:nwflux)
          if (nw>nwflux) pw(igrid)%w4(ixGlo1:ixGhi1,&
             nwflux+1:nw) = pw(igrid)%w(ixGlo1:ixGhi1,nwflux+1:nw)
       end do
       ...
       !$OMP PARALLEL DO PRIVATE(igrid)
       do iigrid=1,igridstail; igrid=igrids(iigrid);
          pw(igrid)%w1(ixGlo1:ixGhi1,1:nwflux) = ( pw(igrid)%w4(ixGlo1:ixGhi1,&
             1:nwflux) - (0.178079954393132d0 * pw(igrid)%w(ixGlo1:ixGhi1,&
             1:nwflux) + 0.821920045606868d0 * pw(igrid)%w3(ixGlo1:ixGhi1,&
       ...
       do iigrid=1,igridstail; igrid=igrids(iigrid);
          pw(igrid)%w(ixGlo1:ixGhi1,1:nwflux)= 0.517231671970585d0 * &
             pw(igrid)%w2(ixGlo1:ixGhi1,1:nwflux) + 0.096059710526147d0 * &
             pw(igrid)%w3(ixGlo1:ixGhi1,1:nwflux) + 0.063692468666290d0 * &
             pw(igrid)%w1(ixGlo1:ixGhi1,1:nwflux) + 0.386708617503269d0 * &
             pw(igrid)%w4(ixGlo1:ixGhi1,1:nwflux)
       end do
       !$OMP END PARALLEL DO
    ...
    case ("rk4")
       ...
       call advect1(flux_scheme,0.5d0, idimmin,idimmax,global_time,0,&
          global_time,1)
       do iigrid=1,igridstail; igrid=igrids(iigrid);
          pw(igrid)%w2(ixGlo1:ixGhi1,1:nwflux)=pw(igrid)%w(ixGlo1:ixGhi1,&
             1:nwflux)
       end do
       call advect1(flux_scheme,0.5d0, idimmin,idimmax,global_time+dt/2d0,1,&
          global_time,2)
       do iigrid=1,igridstail; igrid=igrids(iigrid);
          pw(igrid)%w3(ixGlo1:ixGhi1,1:nwflux)=pw(igrid)%w(ixGlo1:ixGhi1,&
             1:nwflux)
       end do
       ...
       !$OMP PARALLEL DO PRIVATE(igrid)
       do iigrid=1,igridstail; igrid=igrids(iigrid);
          pw(igrid)%w1(ixGlo1:ixGhi1,1:nwflux)=(pw(igrid)%w1(ixGlo1:ixGhi1,&
             1:nwflux) +two*pw(igrid)%w2(ixGlo1:ixGhi1,&
             1:nwflux) +pw(igrid)%w3(ixGlo1:ixGhi1,&
             1:nwflux) -4.0d0*pw(igrid)%w(ixGlo1:ixGhi1,1:nwflux))/3.0d0
       end do
       !$OMP END PARALLEL DO
       ...
       !$OMP PARALLEL DO PRIVATE(igrid)
       do iigrid=1,igridstail; igrid=igrids(iigrid);
          pw(igrid)%w(ixGlo1:ixGhi1,1:nwflux)=pw(igrid)%w1(ixGlo1:ixGhi1,&
    ...
    case ("fourstep")
       ...
       !$OMP PARALLEL DO PRIVATE(igrid)
       do iigrid=1,igridstail; igrid=igrids(iigrid);
          pw(igrid)%w1(ixGlo1:ixGhi1,1:nwflux)=pw(igrid)%w(ixGlo1:ixGhi1,&
             1:nwflux)
       end do
       !$OMP END PARALLEL DO
  ...
  subroutine process1_grid(method,igrid,qdt,ixGmin1,ixGmax1,idimmin,idimmax,&
     qtC,wCT,qt,w,wold)
    use mod_global_parameters
    use mod_fix_conserve
  ...
  subroutine advect1_grid(method,qdt,ixImin1,ixImax1,idimmin,idimmax,qtC,wCT,&
     qt,w,wold,fC,dx1,x)
    ...
    select case (method)
    case ('cd')
       call centdiff(qdt,ixImin1,ixImax1,ixOmin1,ixOmax1,idimmin,idimmax,qtC,&
          wCT,qt,w,fC,dx1,x)
    case ('cd4')
       call centdiff4(qdt,ixImin1,ixImax1,ixOmin1,ixOmax1,idimmin,idimmax,qtC,&
          wCT,qt,w,fC,dx1,x)
    case ('hancock')
       call hancock(qdt,ixImin1,ixImax1,ixOmin1,ixOmax1,idimmin,idimmax,qtC,&
          wCT,qt,w,dx1,x)
    case ('fd')
       call fd(method,qdt,ixImin1,ixImax1,ixOmin1,ixOmax1,idimmin,idimmax,qtC,&
    ...
    case ('tvdmu','tvdlf','hll','hllc','hllcd','hlld')
       ...
       call finite_volume(method,qdt,ixImin1,ixImax1,ixOmin1,ixOmax1,idimmin,&
          idimmax,qtC,wCT,qt,w,wold,fC,dx1,x)
  ...
  subroutine process_advanced(iit,qt)
    ...
    do iigrid=1,igridstail; igrid=igrids(iigrid);
       ...
       if (associated(usr_process_adv_grid)) then
          call usr_process_adv_grid(igrid,level,ixGlo1,ixGhi1,ixMlo1,ixMhi1,&
