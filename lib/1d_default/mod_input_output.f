  ...
  subroutine read_par_files()
    ...
    namelist /meshlist/ refine_max_level,nbufferx1,refine_threshold,&
       derefine_ratio, refine_criterion, stretched_grid, stretched_dim,&
        qstretch_baselevel, amr_wavefilter,max_blocks,block_nx1,domain_nx1,&
       iprob,xprobmin1,xprobmax1, w_refine_weight, prolongprimitive,&
...
107    close(unitpar)
  ...
  subroutine write_snapshot
    ...
    do Morton_no=Morton_start(mype), Morton_stop(mype)
      ...
      if (nwaux>0) then
        ...
        block=>pw(igrid)
        call phys_get_aux(.true., pw(igrid)%w, pw(igrid)%x, ixGlo1,ixGhi1,&
            ixMlo1-1,ixMhi1+1, "write_snapshot")
      endif

      call block_shape_io(igrid, n_ghost, ixOmin1,ixOmax1, n_values)
      ix_buffer(1) = n_values
      ix_buffer(2:) = n_ghost
      w_buffer(1:n_values) = pack(pw(igrid)%w(ixOmin1:ixOmax1, 1:nw), .true.)
            ...
            if (nw_found<nw) then
              if (associated(usr_transform_w)) then
                call usr_transform_w(ixGlo1,ixGhi1,ixMlo1,ixMhi1,nw_found,w,&
                   pw(igrid)%x,pw(igrid)%w)
              else
                pw(igrid)%w(ixOmin1:ixOmax1,1:nw_found)=w(ixOmin1:ixOmax1,&
                   1:nw_found)
              end if
              ...
              if (associated(usr_transform_w)) then
                ...
                pw(igrid)%w(ixOmin1:ixOmax1,1:nw)=w(ixOmin1:ixOmax1,1:nw)
              end if
            else
              pw(igrid)%w(ixOmin1:ixOmax1,1:nw)=w(ixOmin1:ixOmax1,1:nw)
            end if
  ...
  subroutine get_volume_average(power, mode, volume)
    ...
    double precision              :: dvolume(ixGlo1:ixGhi1)
    ...
    do iigrid = 1, igridstail
       ...
       if (slab) then
          ...
          dvolume(ixMlo1:ixMhi1) = pw(igrid)%dvolume(ixMlo1:ixMhi1)
       end if

       ! Store total volume in last element
       wsum(nw+1) = wsum(nw+1) + sum(dvolume(ixMlo1:ixMhi1))

       ! Compute the modes of the cell-centered variables, weighted by volume
       do iw = 1, nw
          wsum(iw) = wsum(iw) + sum(dvolume(ixMlo1:ixMhi1)*pw(igrid)%w(&
             ixMlo1:ixMhi1,iw)**power)
       end do
    end do
  ...
  subroutine get_volume_average_func(func, f_avg, volume)
    ...
    do iigrid = 1, igridstail
       ...
       ! Store total volume in last element
       wsum(2) = wsum(2) + sum(dvolume(ixMlo1:ixMhi1))

       ! Compute the modes of the cell-centered variables, weighted by volume
       do i1 = ixMlo1, ixMhi1
       wsum(1) = wsum(1) + dvolume(i1) * func(pw(igrid)%w(i1, :), nw)
       end do
    end do
