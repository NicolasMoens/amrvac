...
111    close(unitpar)
  ...
  ! Add viscosity source to w within ixO 
    ...
    double precision:: lambda(ixImin1:ixImax1,ndir,ndir),tmp(ixImin1:ixImax1),&
       tmp2(ixImin1:ixImax1),v(ixImin1:ixImax1,ndir)
    ...
    if(qsourcesplit .eqv. vc_split) then
      ...
      ! initialize
      lambda(ixmin1:ixmax1,1:ndir,1:ndir)=zero
      ...
      ! thus gradv_ij=d_j v_i
        ...
        case("limited")
          call gradientS(tmp,ixImin1,ixImax1,ixmin1,ixmax1,idim,tmp2)
        end select
      ...
      do idir=1,ndir
        do idim=1,ndim 
              ...
              case("limited")
                call gradientS(tmp,ixImin1,ixImax1,ixOmin1,ixOmax1,idim,tmp2)
              end select
      ...
      if(energy) then
        ...
        do idim=1,ndim
          tmp=0.d0
          do idir=1,ndir
             tmp(ixmin1:ixmax1)=tmp(ixmin1:ixmax1)+v(ixmin1:ixmax1,&
                idir)*lambda(ixmin1:ixmax1,idir,idim)
          end do
          call gradient(tmp,ixImin1,ixImax1,ixOmin1,ixOmax1,idim,tmp2)
