...
111   close(unitpar)
  ...
  subroutine fields_from_mhd(igrid, w_mhd, w_part)
    ...
    w_part(ixGlo1:ixGhi1,1:ngridvars) = 0.0d0
    ...
    current = zero
    call get_current(w,ixGlo1,ixGhi1,ixGlo1+1,ixGhi1-1,idirmin,current)
    ...
    ! scale to cgs units:
    w_part(ixGlo1:ixGhi1,bp(:)) = w_part(ixGlo1:ixGhi1,&
       bp(:)) * sqrt(4.0d0*dpi*unit_velocity**2 * unit_density)
    w_part(ixGlo1:ixGhi1,ep(:)) = w_part(ixGlo1:ixGhi1,&
       ep(:)) * sqrt(4.0d0*dpi*unit_velocity**2 * unit_density) * &
      ...
      do idir=1,ndir
        call interpolate_var(igrid,ixGlo1,ixGhi1,ixMlo1,ixMhi1,&
           gridvars(igrid)%w(ixGlo1:ixGhi1,ix(idir)),&
            pw(igrid)%x(ixGlo1:ixGhi1,1:ndim),x,vec(idir))
      end do
    else
      do idir=1,ndir
        call interpolate_var(igrid,ixGlo1,ixGhi1,ixMlo1,ixMhi1,&
           gridvars(igrid)%wold(ixGlo1:ixGhi1,ix(idir)),&
            pw(igrid)%x(ixGlo1:ixGhi1,1:ndim),x,vec1(idir))
        call interpolate_var(igrid,ixGlo1,ixGhi1,ixMlo1,ixMhi1,&
           gridvars(igrid)%w(ixGlo1:ixGhi1,ix(idir)),&
            pw(igrid)%x(ixGlo1:ixGhi1,1:ndim),x,vec2(idir))
      end do
