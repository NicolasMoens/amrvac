...
111   close(unitpar)
  ...
  subroutine fields_from_mhd(igrid, w_mhd, w_part)
    ...
    w(ixGlo1:ixGhi1,ixGlo2:ixGhi2,1:nw) = w_mhd(ixGlo1:ixGhi1,ixGlo2:ixGhi2,&
       1:nw)
    w_part(ixGlo1:ixGhi1,ixGlo2:ixGhi2,1:ngridvars) = 0.0d0

    call phys_to_primitive(ixGlo1,ixGlo2,ixGhi1,ixGhi2,ixGlo1,ixGlo2,ixGhi1,&
       ixGhi2,w,pw(igrid)%x)

    ! fill with magnetic field:
    w_part(ixGlo1:ixGhi1,ixGlo2:ixGhi2,bp(:)) = w(ixGlo1:ixGhi1,ixGlo2:ixGhi2,&
       iw_mag(:))

    ! fill with electric field
    current = zero
    call get_current(w,ixGlo1,ixGlo2,ixGhi1,ixGhi2,ixGlo1+1,ixGlo2+1,ixGhi1-1,&
       ixGhi2-1,idirmin,current)
    ...
    ! scale to cgs units:
    w_part(ixGlo1:ixGhi1,ixGlo2:ixGhi2,bp(:)) = w_part(ixGlo1:ixGhi1,&
       ixGlo2:ixGhi2,bp(:)) * sqrt(4.0d0*dpi*unit_velocity**2 * unit_density)
    w_part(ixGlo1:ixGhi1,ixGlo2:ixGhi2,ep(:)) = w_part(ixGlo1:ixGhi1,&
       ixGlo2:ixGhi2,ep(:)) * sqrt(4.0d0*dpi*unit_velocity**2 * unit_density) &
  ...
  subroutine handle_particles()
    ...
    if(physics_type_particles/='advect') &
       tmax_particles=tmax_particles*unit_time
    ...
    if (associated(usr_particle_analytic)) then
      ...
      do idir=1,ndir
        call interpolate_var(igrid,ixGlo1,ixGlo2,ixGhi1,ixGhi2,ixMlo1,ixMlo2,&
           ...
           ix(idir)), pw(igrid)%x(ixGlo1:ixGhi1,ixGlo2:ixGhi2,1:ndim),x,&
           vec(idir))
      end do
    else
      do idir=1,ndir
  ...
  subroutine read_particles_snapshot()
    ...
    if (mype .eq. 0) then
      ...
      if (.not. file_exists) then
        call mpistop('File '//trim(filename)&
...
('npayload in restart file does not match npayload in mod_particles')
  ...
  subroutine init_particles_output()
    ...
    do iipart=1,nparticles_on_mype;ipart=particles_on_mype(iipart);
      if (particle(ipart)%self%follow) then
        filename = make_particle_filename(particle(ipart)%self%t,&
  ...
  subroutine output_particle(myparticle,payload,ipe,file_unit)
    ...
    write(file_unit, csv_format) myparticle%t, myparticle%dt, x(1:3), u,&
  ...
  subroutine comm_particles()
    ...
    do iipart=1,nparticles_on_mype;ipart=particles_on_mype(iipart);
      ...
      if ( .not.time_advance .and. particle(ipart)%self%t .gt. tmax_particles &
         ) then
