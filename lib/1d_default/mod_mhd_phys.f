...
contains
  ...
  subroutine mhd_read_params(files)
    ...
    namelist /mhd_list/ mhd_energy, mhd_n_tracer, mhd_gamma, mhd_adiab,mhd_eta,&
       ...
       mhd_viscosity, mhd_4th_order, typedivbfix, divbdiff, typedivbdiff,&
        compactres, divbwave, He_abundance, SI_unit, B0field,&
...
111    close(unitpar)
  ...
  subroutine mhd_check_w(primitive,ixImin1,ixImax1,ixOmin1,ixOmax1,w,flag)
    use mod_global_parameters
    ...
    if (mhd_energy) then
       if (block%e_is_internal) then
          where(w(ixOmin1:ixOmax1, e_) < small_pressure*inv_gamma_1) &
             flag(ixOmin1:ixOmax1) = e_
       else
         if (primitive)then
           where(w(ixOmin1:ixOmax1, e_) < small_pressure) &
              flag(ixOmin1:ixOmax1) = e_
         else
        ! Calculate pressure=(gamma-1)*(e-0.5*(2ek+2eb))
           tmp(ixOmin1:ixOmax1)=gamma_1*(w(ixOmin1:ixOmax1,e_) - mhd_kin_en(w,&
  ...
  subroutine mhd_to_conserved(ixImin1,ixImax1,ixOmin1,ixOmax1,w,x)
    use mod_global_parameters
    integer, intent(in)             :: ixImin1,ixImax1, ixOmin1,ixOmax1
    ...
    if (mhd_energy) then
      ...
      if(.not.block%e_is_internal) w(ixOmin1:ixOmax1,e_)=w(ixOmin1:ixOmax1,&
         e_) + 0.5d0 * sum(w(ixOmin1:ixOmax1, mom(:))**2,&
          dim=ndim+1) * w(ixOmin1:ixOmax1, rho_) + mhd_mag_en(w, ixImin1,&
    ...
    if (check_small_values) call mhd_handle_small_values(.false., w, x,&
        ixImin1,ixImax1, ixOmin1,ixOmax1,'mhd_to_conserved')
  end subroutine mhd_to_conserved

  !> Transform conservative variables into primitive ones
  subroutine mhd_to_primitive(ixImin1,ixImax1,ixOmin1,ixOmax1,w,x)
    use mod_global_parameters
    integer, intent(in)             :: ixImin1,ixImax1, ixOmin1,ixOmax1
    double precision, intent(inout) :: w(ixImin1:ixImax1, nw)
    double precision, intent(in)    :: x(ixImin1:ixImax1, 1:ndim)
    ...
    if (mhd_energy) then
      if(block%e_is_internal) then
        w(ixOmin1:ixOmax1, p_) = gamma_1*w(ixOmin1:ixOmax1, e_)
      else
       ! Calculate pressure = (gamma-1) * (e-0.5*(2ek+2eb))
        w(ixOmin1:ixOmax1, p_) = gamma_1*(w(ixOmin1:ixOmax1,&
            e_) - mhd_kin_en(w, ixImin1,ixImax1, ixOmin1,ixOmax1,&
            inv_rho) - mhd_mag_en(w, ixImin1,ixImax1, ixOmin1,ixOmax1))
      end if
    end if
    ...
    if (check_small_values) call mhd_handle_small_values(.true., w, x, ixImin1,&
       ixImax1, ixOmin1,ixOmax1,'mhd_to_primitive')
  end subroutine mhd_to_primitive

  subroutine mhd_handle_small_values(primitive, w, x, ixImin1,ixImax1, ixOmin1,&
     ixOmax1, subname)
    use mod_global_parameters
    ...
    call mhd_check_w(primitive, ixImin1,ixImax1, ixOmin1,ixOmax1, w, flag)

    if (any(flag(ixOmin1:ixOmax1) /= 0)) then
       select case (small_values_method)
       case ("replace")
          where(flag(ixOmin1:ixOmax1) /= 0) w(ixOmin1:ixOmax1,&
             rho_) = small_density
  ...
  subroutine e_to_rhos(ixImin1,ixImax1,ixOmin1,ixOmax1,w,x)
    ...
    if (mhd_energy) then
      if(.not.block%e_is_internal) w(ixOmin1:ixOmax1, e_) = w(ixOmin1:ixOmax1,&
          e_) - mhd_kin_en(w, ixImin1,ixImax1, ixOmin1,ixOmax1) - mhd_mag_en(w,&
          ixImin1,ixImax1, ixOmin1,ixOmax1)
      w(ixOmin1:ixOmax1, e_) = gamma_1* w(ixOmin1:ixOmax1,&
          rho_)**(1.0d0 - mhd_gamma) * w(ixOmin1:ixOmax1, e_)
    else
  ...
  subroutine rhos_to_e(ixImin1,ixImax1,ixOmin1,ixOmax1,w,x)
    ...
    if (mhd_energy) then
       w(ixOmin1:ixOmax1, e_) = w(ixOmin1:ixOmax1,&
           rho_)**gamma_1 * w(ixOmin1:ixOmax1, e_) * inv_gamma_1
       if(.not.block%e_is_internal) w(ixOmin1:ixOmax1, e_) =w(ixOmin1:ixOmax1,&
  ...
  !> Calculate v vector
  subroutine mhd_get_v(w,x,ixImin1,ixImax1,ixOmin1,ixOmax1,v)
    use mod_global_parameters
  ...
  subroutine mhd_get_cmax(w,x,ixImin1,ixImax1,ixOmin1,ixOmax1,idim,cmax)
    use mod_global_parameters
    ...
    cmax(ixOmin1:ixOmax1)=abs(w(ixOmin1:ixOmax1,mom(idim))/w(ixOmin1:ixOmax1,&
       rho_))+cmax(ixOmin1:ixOmax1)
  ...
  subroutine mhd_get_cbounds(wLC,wRC,wLp,wRp,x,ixImin1,ixImax1,ixOmin1,ixOmax1,&
    ...
    if (typeboundspeed/='cmaxmean') then
      ...
      tmp3(ixOmin1:ixOmax1)=1.d0/(sqrt(wLp(ixOmin1:ixOmax1,&
         rho_))+sqrt(wRp(ixOmin1:ixOmax1,rho_)))
      ...
      if(present(cmin)) then
        cmin(ixOmin1:ixOmax1)=umean(ixOmin1:ixOmax1)-dmean(ixOmin1:ixOmax1)
        cmax(ixOmin1:ixOmax1)=umean(ixOmin1:ixOmax1)+dmean(ixOmin1:ixOmax1)
      else
        cmax(ixOmin1:ixOmax1)=abs(umean(ixOmin1:ixOmax1))+&
      ...
      call mhd_get_csound(wmean,x,ixImin1,ixImax1,ixOmin1,ixOmax1,idim,&
         csoundR)
      if(present(cmin)) then
        cmax(ixOmin1:ixOmax1)=max(tmp1(ixOmin1:ixOmax1)+&
           csoundR(ixOmin1:ixOmax1),zero)
        cmin(ixOmin1:ixOmax1)=min(tmp1(ixOmin1:ixOmax1)-&
           csoundR(ixOmin1:ixOmax1),zero)
      else
        cmax(ixOmin1:ixOmax1)=abs(tmp1(ixOmin1:ixOmax1))+&
  ...
  subroutine mhd_get_pthermal(w,x,ixImin1,ixImax1,ixOmin1,ixOmax1,pth)
    use mod_global_parameters
  ...
  subroutine mhd_get_csound2(w,x,ixImin1,ixImax1,ixOmin1,ixOmax1,csound2)
    ...
    if(mhd_energy) then
      ...
      csound2(ixOmin1:ixOmax1)=mhd_gamma*csound2(ixOmin1:ixOmax1)/w(&
         ixOmin1:ixOmax1,rho_)
    else
      csound2(ixOmin1:ixOmax1)=mhd_gamma*mhd_adiab*w(ixOmin1:ixOmax1,&
         rho_)**gamma_1
    end if
  ...
  subroutine mhd_get_flux(wC,w,x,ixImin1,ixImax1,ixOmin1,ixOmax1,idim,f)
    ...
    if(mhd_energy) then
      ptotal=w(ixOmin1:ixOmax1,p_)+0.5d0*sum(w(ixOmin1:ixOmax1, mag(:))**2,&
          dim=ndim+1)
    else
    ...
    do idir=1,ndir
      if(idim==idir) then
        f(ixOmin1:ixOmax1,mom(idir))=ptotal(ixOmin1:ixOmax1)-w(ixOmin1:ixOmax1,&
           mag(idim))*w(ixOmin1:ixOmax1,mag(idir))
        if(B0field) f(ixOmin1:ixOmax1,mom(idir))=f(ixOmin1:ixOmax1,&
      ...
      if (B0field) then
        f(ixOmin1:ixOmax1,mom(idir))=f(ixOmin1:ixOmax1,&
           mom(idir))-w(ixOmin1:ixOmax1,mag(idir))*block%B0(ixOmin1:ixOmax1,&
           idim,idim)-w(ixOmin1:ixOmax1,mag(idim))*block%B0(ixOmin1:ixOmax1,&
           idir,idim)
      end if
      f(ixOmin1:ixOmax1,mom(idir))=f(ixOmin1:ixOmax1,&
         mom(idir))+w(ixOmin1:ixOmax1,mom(idim))*wC(ixOmin1:ixOmax1,mom(idir))
    end do
    ...
    if (mhd_energy) then
       if (block%e_is_internal) then
          f(ixOmin1:ixOmax1,e_)=w(ixOmin1:ixOmax1,mom(idim))*w(ixOmin1:ixOmax1,&
             p_)
          if (mhd_Hall) then
          ...
          if (B0field) then
             f(ixOmin1:ixOmax1,e_) = f(ixOmin1:ixOmax1,e_) + w(ixOmin1:ixOmax1,&
                mom(idim)) * tmp(ixOmin1:ixOmax1) - sum(w(ixOmin1:ixOmax1,&
                mom(:))*w(ixOmin1:ixOmax1,mag(:)),&
      ...
      if (idim==idir) then
        ...
        if (B0field) then
          f(ixOmin1:ixOmax1,mag(idir))=f(ixOmin1:ixOmax1,&
             mag(idir))+w(ixOmin1:ixOmax1,mom(idim))*block%B0(ixOmin1:ixOmax1,&
             idir,idim)-w(ixOmin1:ixOmax1,mom(idir))*block%B0(ixOmin1:ixOmax1,&
             idim,idim)
        end if
  ...
  subroutine mhd_add_source(qdt,ixImin1,ixImax1,ixOmin1,ixOmax1,wCT,w,x,&
    ...
    if (.not. qsourcesplit) then
      ...
      if (abs(mhd_eta)>smalldouble)then
        ...
        active = .true.
        if (compactres)then
          call add_source_res1(qdt,ixImin1,ixImax1,ixOmin1,ixOmax1,wCT,w,x)
        else
  ...
  subroutine internal_energy_add_source(qdt,ixImin1,ixImax1,ixOmin1,ixOmax1,&
    ...
    call mhd_get_pthermal(wCT,x,ixImin1,ixImax1,ixOmin1,ixOmax1,pth)
    w(ixOmin1:ixOmax1,e_)=w(ixOmin1:ixOmax1,&
       e_)-qdt*pth(ixOmin1:ixOmax1)*divv(ixOmin1:ixOmax1)
  ...
  subroutine add_source_B0split(qdt,ixImin1,ixImax1,ixOmin1,ixOmax1,wCT,w,x)
    ...
    if(mhd_energy) then
      if(.not.block%e_is_internal) then
        ...
        ! store velocity in a
        do idir=1,ndir
          a(ixOmin1:ixOmax1,idir)=wCT(ixOmin1:ixOmax1,&
             mom(idir))/wCT(ixOmin1:ixOmax1,rho_)
        end do
        call cross_product(ixImin1,ixImax1,ixOmin1,ixOmax1,a,b,axb)
        axb(ixOmin1:ixOmax1,:)=axb(ixOmin1:ixOmax1,:)*qdt
  ...
  subroutine add_source_res1(qdt,ixImin1,ixImax1,ixOmin1,ixOmax1,wCT,w,x)
    ...
    if (mhd_energy) then
       ...
       tmp(ixOmin1:ixOmax1)=zero
       do idir=idirmin,3
          tmp(ixOmin1:ixOmax1)=tmp(ixOmin1:ixOmax1)+current(ixOmin1:ixOmax1,&
             idir)**2
       end do
       w(ixOmin1:ixOmax1,e_)=w(ixOmin1:ixOmax1,&
          e_)+qdt*eta(ixOmin1:ixOmax1)*tmp(ixOmin1:ixOmax1)
    end if
  ...
  subroutine add_source_res2(qdt,ixImin1,ixImax1,ixOmin1,ixOmax1,wCT,w,x)
    ...
    double precision, intent(in)    :: wCT(ixImin1:ixImax1,1:nw),&
        x(ixImin1:ixImax1,1:ndim)
    double precision, intent(inout) :: w(ixImin1:ixImax1,1:nw)
    integer :: ixAmin1,ixAmax1,idir,jdir,kdir,idirmin,iw,idim,idirmin1
    ...
    do jdir=idirmin,3
       tmpvec(ixAmin1:ixAmax1,jdir)=current(ixAmin1:ixAmax1,&
          jdir)*eta(ixAmin1:ixAmax1)
    end do
    call curlvector(tmpvec,ixImin1,ixImax1,ixOmin1,ixOmax1,curlj,idirmin1,1,3)
    do idir=1,ndir
      w(ixOmin1:ixOmax1,mag(idir)) = w(ixOmin1:ixOmax1,&
         mag(idir))-qdt*curlj(ixOmin1:ixOmax1,idir)
    end do
    ...
    if(mhd_energy) then
      ...
      w(ixOmin1:ixOmax1,e_)=w(ixOmin1:ixOmax1,&
         e_)+qdt*(sum(current(ixOmin1:ixOmax1,:)**2,&
         dim=ndim+1)*eta(ixOmin1:ixOmax1)-sum(wCT(ixOmin1:ixOmax1,&
  ...
  subroutine add_source_glm1(qdt,ixImin1,ixImax1,ixOmin1,ixOmax1,wCT,w,x)
    ...
    do idim=1,ndim
       ...
       ! e  = e  -qdt (b . grad(Psi))
         w(ixOmin1:ixOmax1,e_) = w(ixOmin1:ixOmax1,e_)-qdt*wCT(ixOmin1:ixOmax1,&
            mag(idim))*gradPsi(ixOmin1:ixOmax1)
       end if
    end do
    ...
    if (check_small_values) call mhd_handle_small_values(.false.,w,x,ixImin1,&
       ixImax1,ixOmin1,ixOmax1,'add_source_glm1')

  end subroutine add_source_glm1

  subroutine add_source_glm2(qdt,ixImin1,ixImax1,ixOmin1,ixOmax1,wCT,w,x)
    ! Add divB related sources to w within ixO
    ...
    double precision, intent(in) :: qdt,   wCT(ixImin1:ixImax1,1:nw),&
        x(ixImin1:ixImax1,1:ndim)
    double precision, intent(inout) :: w(ixImin1:ixImax1,1:nw)
    double precision:: divb(ixImin1:ixImax1),v(ixImin1:ixImax1,1:ndir)
    integer          :: idim,idir
    double precision :: gradPsi(ixImin1:ixImax1)
    ...
    do idim=1,ndim
      ...
      w(ixOmin1:ixOmax1,psi_) = w(ixOmin1:ixOmax1,psi_)-qdt*v(ixOmin1:ixOmax1,&
         idim)*gradPsi(ixOmin1:ixOmax1)
    ...
    ! e = e - qdt (v . b) * div b
       w(ixOmin1:ixOmax1,e_)=w(ixOmin1:ixOmax1,&
          e_) - qdt * divb(ixOmin1:ixOmax1) * sum(v(ixOmin1:ixOmax1,&
          :)*wCT(ixOmin1:ixOmax1,mag(:)),dim=ndim+1)
    end if
    ! b = b - qdt v * div b
    do idir=1,ndir
      w(ixOmin1:ixOmax1,mag(idir))=w(ixOmin1:ixOmax1,&
    ...
    if (check_small_values) call mhd_handle_small_values(.false.,w,x,ixImin1,&
       ixImax1,ixOmin1,ixOmax1,'add_source_glm2')

  end subroutine add_source_glm2

  subroutine add_source_glm3(qdt,ixImin1,ixImax1,ixOmin1,ixOmax1,wCT,w,x)
    ! Add divB related sources to w within ixO
    ...
    ! dPsi/dt =  - Ch^2/Cp^2 Psi
    if (mhd_glm_alpha < zero) then
      w(ixOmin1:ixOmax1,psi_) = abs(mhd_glm_alpha)*w(ixOmin1:ixOmax1,psi_)
    else
      ! implicit update of Psi variable
      ! equation (27) in Mignone 2010 J. Com. Phys. 229, 2117
      w(ixOmin1:ixOmax1,psi_) = dexp(-qdt*cmax_global*mhd_glm_alpha/minval(&
  ...
  subroutine add_source_janhunen(qdt,ixImin1,ixImax1,ixOmin1,ixOmax1,wCT,w,x)
    ...
    do idir=1,ndir
      w(ixOmin1:ixOmax1,mag(idir))=w(ixOmin1:ixOmax1,&
         mag(idir))-qdt*wCT(ixOmin1:ixOmax1,mom(idir))/wCT(ixOmin1:ixOmax1,&
         rho_)*divb(ixOmin1:ixOmax1)
    end do
  ...
  subroutine get_normalized_divb(w,ixImin1,ixImax1,ixOmin1,ixOmax1,divb)
    ...
    if(slab) then
      divb(ixOmin1:ixOmax1)=0.5d0*abs(divb(ixOmin1:ixOmax1))/sqrt(&
         mhd_mag_en_all(w,ixImin1,ixImax1,ixOmin1,&
         ixOmax1))/sum(1.d0/dxlevel(:))
    else
      ixAmin1=ixOmin1-1;
      ixAmax1=ixOmax1-1;
      ...
      divb(ixOmin1:ixOmax1)=abs(divb(ixOmin1:ixOmax1))/sqrt(mhd_mag_en_all(w,&
         ixImin1,ixImax1,ixOmin1,ixOmax1))*block%dvolume(&
         ixOmin1:ixOmax1)/dsurface(ixOmin1:ixOmax1)
    end if
  ...
  subroutine mhd_add_source_geom(qdt,ixImin1,ixImax1,ixOmin1,ixOmax1,wCT,w,x)
      ...
      if (angmomfix) then
       ...
       if(phi_>0) then
         w(ixOmin1:ixOmax1,mr_)=w(ixOmin1:ixOmax1,mr_)+qdt/x(ixOmin1:ixOmax1,&
            ...
            bphi_)**2+wCT(ixOmin1:ixOmax1,mphi_)**2/wCT(ixOmin1:ixOmax1,rho_))
         w(ixOmin1:ixOmax1,mphi_)=w(ixOmin1:ixOmax1,&
            mphi_)+qdt/x(ixOmin1:ixOmax1,1)*(-wCT(ixOmin1:ixOmax1,&
            mphi_)*wCT(ixOmin1:ixOmax1,mr_)/wCT(ixOmin1:ixOmax1,&
            rho_) +wCT(ixOmin1:ixOmax1,bphi_)*wCT(ixOmin1:ixOmax1,br_))
         w(ixOmin1:ixOmax1,bphi_)=w(ixOmin1:ixOmax1,&
            ...
            br_)*wCT(ixOmin1:ixOmax1,mphi_)) /wCT(ixOmin1:ixOmax1,rho_)
       else
         w(ixOmin1:ixOmax1,mr_)=w(ixOmin1:ixOmax1,mr_)+qdt/x(ixOmin1:ixOmax1,&
    ...
    case ('spherical')
       ...
       if(ndir>1) then
         do idir=2,ndir
           tmp(ixOmin1:ixOmax1)=tmp(ixOmin1:ixOmax1)+wCT(ixOmin1:ixOmax1,&
              mom(idir))**2/wCT(ixOmin1:ixOmax1,rho_)-wCT(ixOmin1:ixOmax1,&
              mag(idir))**2
       ...
       w(ixOmin1:ixOmax1,mom(1))=w(ixOmin1:ixOmax1,&
          mom(1))+qdt*tmp(ixOmin1:ixOmax1)/x(ixOmin1:ixOmax1,1)
       ! b1
       if(mhd_glm) then
       ...
       if(ndir==3) then
         ...
         if(.not.angmomfix) then
           tmp(ixOmin1:ixOmax1)=-(wCT(ixOmin1:ixOmax1,&
              mom(3))*wCT(ixOmin1:ixOmax1,mom(1))/wCT(ixOmin1:ixOmax1,&
              rho_) -wCT(ixOmin1:ixOmax1,mag(3))*wCT(ixOmin1:ixOmax1,mag(1))) 
           if (B0field) then
              tmp(ixOmin1:ixOmax1)=tmp(ixOmin1:ixOmax1)+&
           ...
           w(ixOmin1:ixOmax1,mom(3))=w(ixOmin1:ixOmax1,&
              mom(3))+qdt*tmp(ixOmin1:ixOmax1)/x(ixOmin1:ixOmax1,1)
         else
           call mpistop("angmomfix not implemented yet in MHD")
         end if
         ! b3
         tmp(ixOmin1:ixOmax1)=(wCT(ixOmin1:ixOmax1,mom(1))*wCT(ixOmin1:ixOmax1,&
            mag(3)) -wCT(ixOmin1:ixOmax1,mom(3))*wCT(ixOmin1:ixOmax1,&
            mag(1)))/wCT(ixOmin1:ixOmax1,rho_) 
         if (B0field) then
            tmp(ixOmin1:ixOmax1)=tmp(ixOmin1:ixOmax1)+(wCT(ixOmin1:ixOmax1,&
               mom(1))*block%B0(ixOmin1:ixOmax1,3,0) -wCT(ixOmin1:ixOmax1,&
               mom(3))*block%B0(ixOmin1:ixOmax1,1,0))/wCT(ixOmin1:ixOmax1,&
               rho_)
         end if
  ...
  function mhd_mag_i_all(w, ixImin1,ixImax1, ixOmin1,ixOmax1,idir) result(mgf)
    ...
    if (B0field) then
      mgf = w(ixOmin1:ixOmax1, mag(idir))+block%B0(ixOmin1:ixOmax1,idir,&
         block%iw0)
    else
  ...
  function mhd_kin_en(w, ixImin1,ixImax1, ixOmin1,ixOmax1, inv_rho) result(ke)
    use mod_global_parameters, only: nw, ndim
    integer, intent(in)           :: ixImin1,ixImax1, ixOmin1,ixOmax1
    double precision, intent(in)  :: w(ixImin1:ixImax1, nw)
  ...
  subroutine mhd_getv_Hall(w,x,ixImin1,ixImax1,ixOmin1,ixOmax1,vHall)
    ...
    do idir = idirmin, 3
       vHall(ixOmin1:ixOmax1,idir) = vHall(ixOmin1:ixOmax1,&
          idir)/w(ixOmin1:ixOmax1,rho_)
    end do

  end subroutine mhd_getv_Hall

  subroutine mhd_getdt_Hall(w,x,ixImin1,ixImax1,ixOmin1,ixOmax1,dx1,dthall)
    use mod_global_parameters
    ...
    !.. local ..
    double precision :: dxarr(ndim)
    double precision :: bmag(ixImin1:ixImax1)
    ...
    if (.not. B0field) then
       bmag(ixOmin1:ixOmax1)=sqrt(sum(w(ixOmin1:ixOmax1,mag(:))**2,&
           dim=ndim+1))
       bmag(ixOmin1:ixOmax1)=sqrt(sum((w(ixOmin1:ixOmax1,&
          mag(:)) + block%B0(ixOmin1:ixOmax1,1:ndir,block%iw0))**2))
    end if

    dthall=dtdiffpar*minval(dxarr(1:ndim))**&
       2.0d0/(mhd_etah*maxval(bmag(ixOmin1:ixOmax1)/w(ixOmin1:ixOmax1,rho_)))
  ...
  subroutine glmSolve(wLC,wRC,ixImin1,ixImax1,ixOmin1,ixOmax1,idir)
    use mod_global_parameters
    ...
    dB(ixOmin1:ixOmax1)   = wRC(ixOmin1:ixOmax1,&
       mag(idir)) - wLC(ixOmin1:ixOmax1,mag(idir))
    dPsi(ixOmin1:ixOmax1) = wRC(ixOmin1:ixOmax1,psi_) - wLC(ixOmin1:ixOmax1,&
       psi_)

    wLC(ixOmin1:ixOmax1,mag(idir))   = 0.5d0 * (wRC(ixOmin1:ixOmax1,&
       mag(idir)) + wLC(ixOmin1:ixOmax1,mag(idir))) - 0.5d0/cmax_global * &
       dPsi(ixOmin1:ixOmax1)
    wLC(ixOmin1:ixOmax1,psi_)       = 0.5d0 * (wRC(ixOmin1:ixOmax1,&
       psi_) + wLC(ixOmin1:ixOmax1,psi_)) - 0.5d0*cmax_global * &
       dB(ixOmin1:ixOmax1)

    wRC(ixOmin1:ixOmax1,mag(idir)) = wLC(ixOmin1:ixOmax1,mag(idir))
    wRC(ixOmin1:ixOmax1,psi_) = wLC(ixOmin1:ixOmax1,psi_)
  ...
  subroutine mhd_boundary_adjust
    ...
    integer :: ixGmin1,ixGmax1, ixOmin1,ixOmax1, i1
              ...
              if(any(typeboundary(:,iB)=="special")) then
                ...
                case (1)
                   if (iside==2) then
                      ...
                      ! minimal boundary
                      ixOmin1=ixGmin1;
                      ixOmax1=ixGmin1-1+nghostcells;
                   end if 
                end select
                call fixdivB_boundary(ixGmin1,ixGmax1,ixOmin1,ixOmax1,&
                   pw(igrid)%wb,pw(igrid)%x,iB)
              end if
           end do
        end do
     end do
    ...
    select case(iB)
     case(1)
       ...
       if(mhd_energy.and..not.block%e_is_internal) call &
          mhd_to_primitive(ixGmin1,ixGmax1,ixOmin1,ixOmax1,w,x)
       
       
       if(mhd_energy.and..not.block%e_is_internal) call &
          mhd_to_conserved(ixGmin1,ixGmax1,ixOmin1,ixOmax1,w,x)
     case(2)
       if(mhd_energy.and..not.block%e_is_internal) call &
          mhd_to_primitive(ixGmin1,ixGmax1,ixOmin1,ixOmax1,w,x)
