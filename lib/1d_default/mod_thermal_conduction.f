...
111    close(unitpar)
  ...
  subroutine mhd_get_heatconduct(qd,tmp1,tmp2,ixImin1,ixImax1,ixOmin1,ixOmax1,&
    ...
    if(solve_internal_e) then
      ...
      tmp2(ixImin1:ixImax1) = 0.5d0 * (sum(w(ixImin1:ixImax1,mom(:))**2,&
    ...
    ! B vector
    if(B0field) then
      mf(ixImin1:ixImax1,:)=w(ixImin1:ixImax1,mag(:))+block%B0(ixImin1:ixImax1,&
         :,0)
    else
      mf(ixImin1:ixImax1,:)=w(ixImin1:ixImax1,mag(:));
    end if
    ! |B|
    ...
    if(tc_slope_limiter=='no') then
      ...
      do idims=1,ndim
        ...
        if(tc_perpendicular) gradT(ixCmin1:ixCmax1,&
           ...
           kef(ixCmin1:ixCmax1)*qvec(ixCmin1:ixCmax1,idims)
      end do
      ...
      do idims=1,ndim
        ...
        ixBmin1=ixAmin1;ixBmax1=ixAmax1;
        do ix1=0,1 
           if( ix1==0 .and. 1==idims ) then
             ixBmin1=ixAmin1-ix1; 
             ixBmax1=ixAmax1-ix1; 
  ...
  subroutine mhd_getdt_heatconduct(w,ixImin1,ixImax1,ixOmin1,ixOmax1,dtnew,dx1,&
    ...
    ! B
    if(B0field) then
      mf(ixOmin1:ixOmax1,:)=w(ixOmin1:ixOmax1,mag(:))+block%B0(ixOmin1:ixOmax1,&
         :,0)
    else
      mf(ixOmin1:ixOmax1,:)=w(ixOmin1:ixOmax1,mag(:))
    end if
    ! B^-2
  ...
  subroutine hd_get_heatconduct(tmp,tmp1,tmp2,ixImin1,ixImax1,ixOmin1,ixOmax1,&
     w,x,qvec)
    use mod_global_parameters
    use mod_small_values, only: small_values_method
    ...
    double precision, intent(out) :: tmp(ixImin1:ixImax1),&
       tmp1(ixImin1:ixImax1),tmp2(ixImin1:ixImax1)
    double precision :: qvec(ixImin1:ixImax1,1:ndim),Te(ixImin1:ixImax1),&
       qflux(ixImin1:ixImax1),qsatflux(ixImin1:ixImax1)
    integer:: ixmin1,ixmax1,idims,ix1
    integer, dimension(ndim)       :: lowindex
    
    ixmin1=ixOmin1-2;ixmax1=ixOmax1+2;
    if (ixImin1>ixmin1.or.ixImax1<ixmax1) call &
       mpistop("Need two extra layers for thermal conduction")
    
    ! store old kinetic energy
    tmp2(ixImin1:ixImax1)=half*sum(w(ixImin1:ixImax1,mom(:))**2,&
       dim=ndim+1)/w(ixImin1:ixImax1,rho_)
    ! store old internal energy
    tmp1(ixImin1:ixImax1)=w(ixImin1:ixImax1,e_)-tmp2(ixImin1:ixImax1)
    ! Clip off negative pressure if small_pressure is set
    if(small_values_method=='error' .and. .not.crash) then
      if(any(tmp1(ixImin1:ixImax1)<small_e)) then
        lowindex=minloc(tmp1(ixImin1:ixImax1))
    ...
    do idims=1,ndim
      ! idirth component of gradient of temperature at cell center
       ...
       case("central")
         ixmin1=ixImin1+1;ixmax1=ixImax1-1;
         call gradient(Te,ixImin1,ixImax1,ixmin1,ixmax1,idims,tmp)
       case("limited")
         ixmin1=ixImin1+2;ixmax1=ixImax1-2;
         call gradientS(Te,ixImin1,ixImax1,ixmin1,ixmax1,idims,tmp)
       end select
       qvec(ixmin1:ixmax1,idims)=tmp(ixmin1:ixmax1)*tc_k_para*dsqrt(Te(&
          ixmin1:ixmax1)**5)
    end do

    if(tc_saturate) then
      ! consider saturation with unsigned saturated TC flux = 5 phi rho c**3
      qsatflux(ixmin1:ixmax1)=5.d0*w(ixmin1:ixmax1,&
         rho_)*dsqrt(Te(ixmin1:ixmax1)**3)
      qflux(ixmin1:ixmax1)=dsqrt(qvec(ixmin1:ixmax1,1)**2)
      do ix1=ixmin1,ixmax1
        if(qflux(ix1)>qsatflux(ix1)) then
          qsatflux(ix1)=qsatflux(ix1)/qflux(ix1)
          do idims=1,ndim
            qvec(ix1,idims)=qsatflux(ix1)*qvec(ix1,idims)
          end do
        end if
      end do
    end if
    ...
    select case(typediv)
      case("central")
        call divvector(qvec,ixImin1,ixImax1,ixOmin1,ixOmax1,tmp)
      case("limited")
        call divvectorS(qvec,ixImin1,ixImax1,ixOmin1,ixOmax1,tmp)
    end select
