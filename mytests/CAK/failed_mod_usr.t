...
!==========================================================================================
  ...
  subroutine special_source(qdt,ixI^L,ix^L,iw^LIM,qtC,wCT,qt,w,x)
    ...
    v(ixI^S) = wCT(ixI^S,mom(1))/wCT(ixI^S,rho_)
    ...
    do i = ixmin1+1,ixmax1-1 
      ! forward difference
      grad_fwd(i) = abs( (v(i+1)-v(i))/(x(i+1,1)-x(i,1)) )
      ! backward difference
      grad_bwd(i) = abs( (v(i) - v(i-1))/(x(i,1)-x(i-1,1)) )
      ! central difference
      grad_centCT(i) = 0.5*(grad_fwd(i)+grad_bwd(i))
    end do
      grad_centCT(ixmin1) = abs( (v(ixmin1+1) - v(ixmin1))/(x(ixmin1+1,1)-x(ixmin1,1)) )
      grad_centCT(ixmax1) = abs( (v(ixmax1) - v(ixmax1-1))/(x(ixmax1,1)-x(ixmax1-1,1)) )
    
    !--------------------------------------------------------------------------------------
    
    do i = ixImin1, ixImax1
      beta_fd(i) = (1 - (v(i)/x(i,1))/grad_centCT(i))*(radius/x(i,1))**2
    ...
    !v = v + qdt*g
    v(ixI^S) = v(ix^S) + qdt*(g_CAK(ix^S) - g_grav_sc(ix^S))
    
    w(ixI^S,mom(1)) =   v(ix^S)*w(ix^S,rho_)  
