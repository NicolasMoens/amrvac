...
contains
  ...
  subroutine read_par_files()
    ...
    namelist /methodlist/ time_integrator, source_split_usr,typesourcesplit,&
       dimsplit,typedimsplit,flux_scheme,typepred1,limiter,gradient_limiter,&
       cada3_radius,loglimit,typelimited,typeboundspeed, typetvd,typeentropy,&
       entropycoef,typeaverage, typegrad,typediv,nxdiffusehllc, flathllc,&
        tvdlfeps,flatcd,flatsh,flatppm,small_temperature,small_pressure,&
       small_density, small_values_method, small_values_daverage,&
        check_small_values, solve_internal_e, angmomfix
    ...
    namelist /meshlist/ refine_max_level,nbufferx1,nbufferx2,refine_threshold,&
       derefine_ratio, refine_criterion, stretched_grid, stretched_dim,&
        qstretch_baselevel, amr_wavefilter,max_blocks,block_nx1,block_nx2,&
       domain_nx1,domain_nx2,iprob,xprobmin1,xprobmin2,xprobmax1,xprobmax2,&
...
107    close(unitpar)
    ...
    do level=1,nlevelshi
       if(flux_scheme(level)=='tvd'.and.time_integrator/='onestep') call &
          mpistop(" tvd is onestep method, reset time_integrator='onestep'")
       ...
       if(flux_scheme(level)=='hlld'.and.physics_type/='mhd') call &
          mpistop("Cannot use hlld flux if not using MHD physics!")
    ...
    if (any(typeboundary_min1 /= undefined)) then
      typeboundary(1:nwfluxbc, 2*1-1) = typeboundary_min1(1:nwfluxbc)
    end if

    if (any(typeboundary_max1 /= undefined)) then
      typeboundary(1:nwfluxbc, 2*1) = typeboundary_max1(1:nwfluxbc)
    end if
    
    if (any(typeboundary_min2 /= undefined)) then
      typeboundary(1:nwfluxbc, 2*2-1) = typeboundary_min2(1:nwfluxbc)
    end if

    if (any(typeboundary_max2 /= undefined)) then
      typeboundary(1:nwfluxbc, 2*2) = typeboundary_max2(1:nwfluxbc)
    end if
    ...
    if (any(nx_vec < 2) .or. any(mod(nx_vec,&
        2) == 1)) call mpistop&
       ('Grid size (domain_nx^D) has to be even and positive')
    ...
    if(stretched_grid) then
       allocate(qstretch(0:nlevelshi),dxfirst(0:nlevelshi),&
          dxfirst_1mq(0:nlevelshi)) 
       if(qstretch_baselevel<1.0d0.or.qstretch_baselevel==bigdouble) then 
          ...
          if(slab_stretched) then
             if(xprobmin2>smalldouble)then
                qstretch_baselevel=(xprobmax2/xprobmin2)**&
             ...
             if(xprobmin1>smalldouble)then
                qstretch_baselevel=(xprobmax1/xprobmin1)**&
       ...
       if(slab_stretched) then
          if(mod(block_nx2,2)==1) call mpistop(&
             "stretched grid needs even block size block_nxND")
          if(mod(domain_nx2,block_nx2)/=0) call &
             mpistop("block size for ND must divide domain size")
          if(mod(domain_nx2/block_nx2,2)/=0) call &
             mpistop("number level 1 blocks in ND must be even")
          dxfirst(1)=(xprobmax2-xprobmin2) &
             *(1.0d0-qstretch(1))/(1.0d0-qstretch(1)**domain_nx2)
       else
          if(mod(block_nx1,2)==1) call mpistop(&
             "stretched grid needs even block size block_nx1")
          if(mod(domain_nx1,block_nx1)/=0) call &
             mpistop("block size must divide domain size")
          if(mod(domain_nx1/block_nx1,2)/=0) call &
             mpistop("number level 1 blocks must be even")
       ...
       if(mype==0) then
          write(*,*) 'Using stretched grid with qs=',&
             qstretch(0:refine_max_level)
          write(*,*) '        and first cell sizes=',&
             dxfirst(0:refine_max_level)
       endif
    end if
  ...
  subroutine snapshot_write_header(fh, offset_tree, offset_block)
    ...
    call MPI_FILE_WRITE(fh, [ block_nx1,block_nx2 ], ndim, MPI_INTEGER, st,&
        er)
    do iw = 1, nw
      call MPI_FILE_WRITE(fh, cons_wnames(iw), name_len, MPI_CHARACTER, st,&
          er)
    end do
  ...
  subroutine block_shape_io(igrid, n_ghost, ixOmin1,ixOmin2,ixOmax1,ixOmax2,&
    ...
    if(save_physical_boundary) then
      do idim=1,ndim
        ...
        if(pw(igrid)%is_physical_boundary(2*idim)) &
  ...
  subroutine write_snapshot
    ...
    if (mype==0) then
      ...
      if (offset_block_data - offset_tree_info /= (nleafs + nparents) * &
         size_logical + nleafs * ((2+ndim) * size_int)) then
    ...
    do Morton_no=Morton_start(mype), Morton_stop(mype)
      ...
      if (nwaux>0) then
        ...
        block=>pw(igrid)
        call phys_get_aux(.true., pw(igrid)%w, pw(igrid)%x, ixGlo1,ixGlo2,&
           ixGhi1,ixGhi2, ixMlo1-1,ixMlo2-1,ixMhi1+1,ixMhi2+1,&
            "write_snapshot")
      endif
      ...
      ix_buffer(2:) = n_ghost
      w_buffer(1:n_values) = pack(pw(igrid)%w(ixOmin1:ixOmax1,ixOmin2:ixOmax2,&
          1:nw), .true.)
    ...
    if (mype == 0) then
      do ipe = 1, npe-1
        do Morton_no=Morton_start(ipe), Morton_stop(ipe)
          ...
          ! Set offset of next block
          block_offset(iwrite+1) = block_offset(iwrite) + n_values * &
             size_double + 2 * ndim * size_int
        end do
      end do
    ...
    if (mype==0) then
            ...
            if (nw_found<nw) then
              if (associated(usr_transform_w)) then
                call usr_transform_w(ixGlo1,ixGlo2,ixGhi1,ixGhi2,ixMlo1,ixMlo2,&
                   ixMhi1,ixMhi2,nw_found,w,pw(igrid)%x,pw(igrid)%w)
              else
                pw(igrid)%w(ixOmin1:ixOmax1,ixOmin2:ixOmax2,&
      ...
      do Morton_no=Morton_start(mype),Morton_stop(mype)
        ...
        if (nw_found<nw) then
          ...
          pw(igrid)%w(ixOmin1:ixOmax1,ixOmin2:ixOmax2,1:nw)=w(ixOmin1:ixOmax1,&
    ...
    if (mype==0)then
      ...
      do Morton_no=Morton_start(0),Morton_stop(0)
        ...
        offset=int(size_block_io,kind=MPI_OFFSET_KIND) *int(Morton_no-1,&
           kind=MPI_OFFSET_KIND)
        call MPI_FILE_READ_AT(fh,offset,pw(igrid)%w,1,type_block_io, istatus,&
           ierrmpi)
      end do
      if (npe>1) then
        do ipe=1,npe-1
  ...
  subroutine get_volume_average(power, mode, volume)
    ...
    do iigrid = 1, igridstail
       ...
       if (slab) then
          dvolume(ixMlo1:ixMhi1,ixMlo2:ixMhi2) = rnode(rpdx1_,&
             igrid)*rnode(rpdx2_,igrid)
       else
          dvolume(ixMlo1:ixMhi1,ixMlo2:ixMhi2) = &
             pw(igrid)%dvolume(ixMlo1:ixMhi1,ixMlo2:ixMhi2)
       end if

       ! Store total volume in last element
       wsum(nw+1) = wsum(nw+1) + sum(dvolume(ixMlo1:ixMhi1,ixMlo2:ixMhi2))

       ! Compute the modes of the cell-centered variables, weighted by volume
       do iw = 1, nw
          wsum(iw) = wsum(iw) + sum(dvolume(ixMlo1:ixMhi1,&
             ixMlo2:ixMhi2)*pw(igrid)%w(ixMlo1:ixMhi1,ixMlo2:ixMhi2,&
             iw)**power)
       end do
    end do
  ...
  subroutine get_volume_average_func(func, f_avg, volume)
    ...
    do iigrid = 1, igridstail
       ...
       ! Store total volume in last element
       wsum(2) = wsum(2) + sum(dvolume(ixMlo1:ixMhi1,ixMlo2:ixMhi2))

       ! Compute the modes of the cell-centered variables, weighted by volume
       do i1 = ixMlo1, ixMhi1
       do i2 = ixMlo2, ixMhi2
       wsum(1) = wsum(1) + dvolume(i1,i2) * func(pw(igrid)%w(i1,i2, :), nw)
       end do
       end do
    end do
  ...
  subroutine get_global_maxima(wmax)
    ...
    do iigrid = 1, igridstail
       igrid = igrids(iigrid)
       do iw = 1, nw
          wmax_mype(iw)=max(wmax_mype(iw),maxval(pw(igrid)%w(ixMlo1:ixMhi1,&
             ixMlo2:ixMhi2,iw)))
       end do
    end do
  ...
  subroutine get_global_minima(wmin)
    ...
    do iigrid = 1, igridstail
       igrid = igrids(iigrid)
       do iw = 1, nw
          wmin_mype(iw)=min(wmin_mype(iw),minval(pw(igrid)%w(ixMlo1:ixMhi1,&
             ixMlo2:ixMhi2,iw)))
       end do
    end do
