...
subroutine lohner_grid(igrid)
  ...
  if(B0field) then
    if(phys_energy) w(ixGlo1:ixGhi1,ixGlo2:ixGhi2,iw_e)=w(ixGlo1:ixGhi1,&
       ixGlo2:ixGhi2,iw_e)+0.5d0*sum(pw(igrid)%B0(ixGlo1:ixGhi1,ixGlo2:ixGhi2,&
     ...
     if (iflag > nw) then
        if (.not. associated(usr_var_for_errest)) then
           ...
           call usr_var_for_errest(ixGlo1,ixGlo2,ixGhi1,ixGhi2,ixGlo1,ixGlo2,&
              ixGhi1,ixGhi2,iflag,pw(igrid)%w,pw(igrid)%x,tmp1)
        end if
     end if
  ...
  do ix1=ixMlo1,ixMhi1
     ...
     if (associated(usr_refine_threshold)) then
        ...
        xtol(1:ndim) = pw(igrid)%x(ix1,ix2,1:ndim)
...
do ix1=ixMlo1,ixMhi1
   ...
   if (associated(usr_refine_threshold)) then
      wtol(1:nw)   = pw(igrid)%w(ix1,ix2,1:nw)
      xtol(1:ndim) = pw(igrid)%x(ix1,ix2,1:ndim)
      call usr_refine_threshold(wtol, xtol, threshold, global_time)
   end if
...
if (associated(usr_refine_grid)) then
   call usr_refine_grid(igrid,level,ixGlo1,ixGlo2,ixGhi1,ixGhi2,ixMlo1,ixMlo2,&
      ixMhi1,ixMhi2,qt,w,pw(igrid)%x, my_refine,my_coarsen)
end if
